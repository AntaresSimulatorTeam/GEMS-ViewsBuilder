...
# TODO(#57): view_config.yml ──[taxonomy field, planned]──► taxonomy.yml  (id must match + taxonomy-category must exist in taxonomy, not yet implemented)
...
%%             enabling: scope.taxonomy-category ∈ taxonomy.categories[*].id,
%%             view_config.taxonomy==catalog.taxonomy, and
