...
def test_filter_simulation_table_logical(tmp_path: Path, test_dataset_dir: Path) -> None:
    ...
    simulation_table_file = next(iter(sorted((test_dataset_dir / "output").glob("*/simulation_table*.parquet"))))
    calendar = load_calendar(test_dataset_dir / "input" / "calendar_file.csv")
...
def test_filter_simulation_table_drops_mismatched_block(tmp_path: Path, test_dataset_dir: Path) -> None:
    ...
    calendar = load_calendar(test_dataset_dir / "input" / "calendar_file.csv")
    base_sim_table_file = next(iter(sorted((test_dataset_dir / "output").glob("*/simulation_table*.parquet"))))
