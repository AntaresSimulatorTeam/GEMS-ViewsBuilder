...
def property_order_workspace(test_files_root: Path, tmp_path: Path) -> tuple[Path, pl.DataFrame]:
    ...
    run_view_building_process(layout_from_dataset(dst), ParquetViewSinker(results_dir))
...
def test_breakdown_missing_property_keys_use_none_literal(test_files_root: Path) -> None:
    ...
    library_path = root / "input" / "model-libraries" / "library.yml"
    library = load_library(library_path)
    system = load_system(root / "input" / "system.yml", resolve_libraries(library_path))
