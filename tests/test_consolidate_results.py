...
# Schema matching _aggregate_metric_temporally output
_CHUNK_SCHEMA = {
    "metric_id": pl.Utf8,
...
def _make_builder(tmp_path: Path) -> ViewBuilder:
    vb = object.__new__(ViewBuilder)
    vb.input_data_path = tmp_path
    return vb
...
def test_parquet_row_group_size_respected(tmp_path: Path) -> None:
    ...
    chunks_dir = tmp_path / "temporal_aggregation"
    chunks_dir.mkdir()
    n_rows = _PARQUET_ROW_GROUP_SIZE + 1
    chunk = _write_chunk(
