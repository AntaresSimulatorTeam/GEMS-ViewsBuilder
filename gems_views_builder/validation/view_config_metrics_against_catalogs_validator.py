...
class ViewConfigMetricsAgainstCatalogsValidator:
    ...
    def validate(self) -> None:
        logging.info(f"Validating view config {self.view_config.id!r} metrics against {len(self.catalogs)} catalog(s)")
        self._validate_metric_ids()
        logging.info("All metrics are present in the catalogs")

    def _validate_metric_ids(self) -> None:
        for metric_ref in self.view_config.metric_ids:
            catalog_id, metric_id = metric_ref.split(".")

            if catalog_id not in self.catalogs:
                raise ValueError(f"Catalog {catalog_id} used in view config {self.view_config.id!r} doesn't exist!")

            if metric_id not in self.catalogs[catalog_id].metrics:
                raise ValueError(
                    f"Metric {metric_id} used in view config {self.view_config.id!r} doesn't exist in catalog {catalog_id!r}"
                )
